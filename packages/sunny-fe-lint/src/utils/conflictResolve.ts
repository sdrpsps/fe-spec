...
export default async (cwd: string, rewriteConfig?: boolean) => {
  ...
  if (willChangeCount > 0) {
    logs.warn(`检测到项目中存在可能与 ${PACKAGE_NAME} 冲突的依赖和配置，为保证正常运行将`);

    if (willRemovePackage.length > 0) {
      logs.warn('删除以下依赖：');
      logs.warn(JSON.stringify(willRemovePackage, null, 2));
    }

    if (uselessConfig.length > 0) {
      logs.warn('删除以下配置文件：');
      logs.warn(JSON.stringify(uselessConfig, null, 2));
    }

    if (reWriteConfig.length > 0) {
      logs.warn('覆盖以下配置文件：');
      logs.warn(JSON.stringify(reWriteConfig, null, 2));
    }
